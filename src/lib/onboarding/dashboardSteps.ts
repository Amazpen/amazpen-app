...
const DAILY_ENTRY_DESCRIPTION = `<div>כאן מזינים את הנתונים היומיים של העסק שנבחר: ההכנסה של היום, אמצעי התשלום, כמות ההזמנות ועלות העובדים. הזנה יומית עקבית היא הבסיס של כל המערכת, שכן ממנה נבנים כל הסיכומים, הגרפים וההשוואות ליעדים. בעזרת הכפתור 'הצגת ועריכת נתונים' אפשר לחזור אחורה, לבדוק ימים קודמים ולתקן טעויות.</div><div style="margin-top:8px;">שים לב: עלות העובדים שמזינים כאן היא <b>הערכה</b> למהלך החודש. בסוף החודש, בדוח רווח והפסד, לוחצים 'סגור חודש' ומזינים את הסכומים שיצאו <b>בפועל</b> מהנהלת החשבונות (שכר, פנסיה, ביטוח לאומי, פיצויים), והם נכנסים לתזרים כתשלומים אמיתיים שדורסים את ההערכה.</div><div style="margin-top:8px;">כך נראה הטופס בפועל:</div>${DAILY_ENTRY_FORM_DEMO}`;
...
export const dashboardSteps: DriveStep[] = [
  ...
  },
  {
    ...
    popover: {
      ...
      description:
        "אזור ההכנסות מציג תחילה את כרטיס 'סך ההכנסות' הכולל של העסק, ולצדו כרטיסים נוספים לכל מקור הכנסה שהוגדר לעסק. חלק מהכרטיסים קבועים לכל העסקים, וחלקם משתנים או מתווספים לפי מקורות ההכנסה הספציפיים שהגדרת (למשל מנות, משלוחים או שירותים). לצד כל כרטיס מוצגת השוואה ליעד ולתקופה הקודמת, ולחיצה על כרטיס פותחת היסטוריה מפורטת.",
      side: "top",
      align: "center",
    },
  },
  {
    ...
    popover: {
      ...
      description:
        "אזור ההוצאות מרכז את הכרטיסים הקבועים כמו עלות העובדים ועלות המכר, ולצדם כרטיסים דינמיים לכל מוצר מנוהל שהגדרת לעסק. גם כאן, חלק מהכרטיסים זהים לכל העסקים וחלקם מותאמים אישית לעסק שלך. כל כרטיס מציג את הסכום בפועל לצד היעד והשוואה לתקופה הקודמת, כדי לעזור לך לאתר היכן ניתן להתייעל ולשפר את הרווחיות.",
      side: "top",
      align: "center",
    },
  },
