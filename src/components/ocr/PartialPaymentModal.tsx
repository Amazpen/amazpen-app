  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="bg-[#0f1535] border-[#4C526B] text-white rounded-[20px] p-[20px] sm:max-w-[560px]" dir="rtl">
        ...
        {/* Mounted only while open, so the body seeds its state from props in
            useState initialisers instead of a re-seeding effect. The caller
            builds `invoices` inline (.filter().map()), so an effect keyed on it
            wiped the amount the user typed on every parent re-render. */}
        {open && (
          <PartialPaymentModalBody
            invoices={invoices}
...
function PartialPaymentModalBody({
  invoices,
  initialAmount,
  onClose,
  onConfirm,
  ...
  // Seeded once per open: all invoices checked, amount = initialAmount.
  ...
  const preview = allocatePartialPayment(
    checkedOrdered.map((i) => ({ id: i.id, balance: i.balance })),
    paymentAmount
  );
